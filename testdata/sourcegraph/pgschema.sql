...
CREATE FUNCTION public.func_configuration_policies_transition_columns_diff(old configuration_policies_transition_columns, new configuration_policies_transition_columns) RETURNS hstore[]
    LANGUAGE plpgsql
    AS $$
    BEGIN
...
CREATE FUNCTION public.func_lsif_uploads_transition_columns_diff(old lsif_uploads_transition_columns, new lsif_uploads_transition_columns) RETURNS hstore[]
    LANGUAGE plpgsql
    AS $$
    BEGIN
        -- array || NULL should be a noop, but that doesn't seem to be happening
...
CREATE FUNCTION public.func_row_to_configuration_policies_transition_columns(rec record) RETURNS configuration_policies_transition_columns
    LANGUAGE plpgsql
    AS $$
    BEGIN
        RETURN (
...
CREATE FUNCTION public.func_row_to_lsif_uploads_transition_columns(rec record) RETURNS lsif_uploads_transition_columns
    LANGUAGE plpgsql
    AS $$
    BEGIN
