...
func TestComplexRealWorldViews(t *testing.T) {
  ...
  GROUP BY emp_no`,
   ...
   FROM employees`,
		...
		{
			name: "view with multiple CTEs (limitation)",
			sql1: ` WITH high_earners AS (SELECT emp_no FROM employees WHERE salary > 75000), dept_counts AS (SELECT dept_id, count(*) AS cnt FROM employees GROUP BY dept_id) SELECT h.emp_no, d.cnt FROM high_earners h JOIN dept_counts d ON true`,
			sql2: ` WITH high_earners AS (
    SELECT emp_no 
    FROM employees 
    WHERE salary > 75000
), dept_counts AS (
...
SELECT h.emp_no, d.cnt 
FROM high_earners h 
JOIN dept_counts d ON true`,
			expectEqual: false, // CTEs not yet supported in comparison logic
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != tt.expectEqual {
				...
				if strings.Contains(tt.name, "limitation") {
					t.Logf("Expected limitation: This test documents current parser limitations")  
					t.Logf("SQL 1:\n%s", tt.sql1)
					t.Logf("SQL 2:\n%s", tt.sql2)
				} else {
					t.Errorf("compareViewDefinitionsSemanticially() = %v, expected %v", result, tt.expectEqual)
