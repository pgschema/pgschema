...
- **No Shadow Database Required**: Unlike other declarative tools, pgschema works directly with your schema files and target database - no temporary databases, no extra infrastructure.
...
**Multi-Tenant Architecture Support**: For applications using multiple schemas, the predominant pattern is **schema-per-tenant architecture** - where each customer or tenant gets their own schema within the same database. pgschema's schema-level approach enables [tenant schema reconciliation](/workflow/tenant-schema) - ensuring all tenant schemas stay in sync with the canonical schema definition.
...
The `pgschema` workflow mirrors Terraform's approach and fits [GitOps](/workflow/gitops) nicely:
...
This compares your desired schema (from the file) with the current database state and generates a migration plan. Here's where `pgschema` differentiates from Terraform—the plan supports multiple output formats:
...
`pgschema` solves this with **fingerprinting** - a cryptographic mechanism that ensures the exact database state you planned against is the same state you're applying changes to.
...
`pgschema` automatically determines transaction boundaries based on the SQL operations in your migration plan. Each diff in the plan contains a `can_run_in_transaction` field that indicates whether that particular SQL can run inside a transaction.
...
`pgschema` operates at the schema level, among all the DDL operations that `pgschema` supports, only `CREATE INDEX CONCURRENTLY` cannot run inside a transaction.

## Modular Schema Organization

For large applications and teams, managing your database schema as a single monolithic file becomes unwieldy. The [modular](/workflow/modular-schema-files) option transforms your schema into an organized structure that enables better collaboration, clearer ownership, and easier maintenance.

### Breaking Down the Monolith

Instead of a single large schema file, `pgschema` can organize your database objects into logical, manageable files with the [`--multi-file`](/cli/dump#param-multi-file) option:
...
Database objects often depend on each other—triggers need functions, views reference tables, foreign keys require referenced tables to exist first. `pgschema` automatically handles these dependencies using topological sorting to ensure operations execute in the correct order, regardless of how your schema files are organized.
...
### IR Normalization
`pgschema` eliminates these complexities by working directly with your schema files and target database through an **Intermediate Representation (IR)** system:
...
- [pg_query_go](https://github.com/pganalyze/pg_query_go) - Go bindings for libpg_query, enabling reliable Postgres SQL parsing
...
- [pg-schema-diff](https://github.com/stripe/pg-schema-diff) - Reading GitHub issues to understand use cases and studying the codebase to learn the trick of leveraging sqlc for query generation
...
Ready to ditch imperative migrations? Get started with `pgschema` and experience the declarative Postgres workflow you've been waiting for.
