...
# Or apply without confirmation (for automated deployments)
pgschema apply --host localhost --db myapp --user postgres --file schema.sql --auto-approve
```

## CI/CD Integration

## Concurrent Changes with Fingerprinting
...
1. **Plan Storage**: The fingerprint is embedded in the plan output and schema file metadata
